...
# Configure SDL for Wayland/Sway compatibility
...
class Simulation:
    ...
    def score_car(self, car, ground, visualize=False):
        ...
        if visualize:
            ...
            pygame.init()
            # Ensure we create a proper display window, not just a surface
            ...
            if visualize:
                ...
                if not nan_detected:
                    try:
                        ...
                        pygame.display.flip()  # This should work now
                        clock.tick(fps)
                    ...
                    # Add a small delay even when not drawing to prevent CPU spinning
                    clock.tick(fps)
